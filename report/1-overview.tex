...
In this project, we reformulate IDC as a sequential decision-making problem and optimize caption generation using on-policy reinforcement learning. Specifically, we investigate Proximal Policy Optimization (PPO) to directly maximize sequence-level rewards that capture visual faithfulness and descriptive precision \citep{schulman2017proximalpolicyoptimizationalgorithms}. Additionally, we incorporate a contrastive objective to enforce alignment between generated captions and visual differences in embedding space \citep{radford2021learning}. We evaluate our approach on standard IDC benchmarks and compare against strong preference-based baselines.

\paragraph{TL;DR (``Too Long; Didn't Read"):}

We reformulate IDC as an MDP and replace DPO with on-policy RL (PPO) to improve robustness and fine-grained visual grounding.
...
\item \textbf{State-of-the-art methods.} ADPO represents the strongest baseline for IDC using preference optimization with adversarial negatives. \textbf{PPO is a standard and stable reinforcement learning method for sequence generation}, providing a robust framework for optimizing language policies through actor-critic architectures.

\end{itemize}

\subsubsection*{Expected Contribution of Each Team Member}

\begin{itemize}
\item \textbf{Eason Liou}: empirical evaluation, dataset selection and processing
\item \textbf{Jia Chi Liou}: reward design, RLHF experiment setting and fine-tune parameters
\item \textbf{Imrose Batterywala}: VLM experiment environment setting, result visualization and literature review
